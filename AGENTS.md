...
Kaçış ilerlemesi `scripts/maturity.sh` ile ölçülür (5 kategori, 50 puan, eşik 40). Her iterasyonun sonunda `scripts/validate.sh` ile doğrulama yap ve `scripts/maturity.sh` ile skoru güncelle; skoru artırmaya çalış.
...
8. Değişikliklerden sonra `scripts/validate.sh` hatasız çalışmalıdır.
