...
| 3         | 2026-08-16 | opencode.json şema uyumsuzluğu giderildi (geçersiz anahtarlar kaldırıldı). `scripts/validate.py` ile otomatik sağlık doğrulaması ve olgunluk puanı oluşturuldu. `ci.yml` workflow'u eklendi. VERSION dosyası ve sürüm senkronizasyonu eklendi. README "Proje Yapısı" ve "Kaçış" bölümleriyle güncellendi. Olgunluk puanı: %100 — kaçış menziline ulaşıldı. |
